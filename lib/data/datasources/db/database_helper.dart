...
class DatabaseHelper {
  ...
  Future<Database> _initDb() async {
    ...
    var db = await openDatabase(databasePath,
        version: 1,
        onCreate: _onCreate,
        password: encrypt('aplikasi_berita_c'));
    return db;
  }
  ...
  Future<int> insertBookmarkArticle(ArticleTable articles) async {
    final db = await database;
  ...
  Future<int> removeBookmarkArticle(ArticleTable articles) async {
    final db = await database;
    return await db!.delete(
      _tblBookmarkArticle,
      where: 'url = ?',
      whereArgs: [articles.url],
    );
  }
